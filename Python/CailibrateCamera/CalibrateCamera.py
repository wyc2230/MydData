...
if __name__ == 'main':
    ...
    mtx, dist, K, D = calibrate_single(30,37,9,6)

    width, height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT))

    p = cv2.fisheye.estimateNewCameraMatrixForUndistortRectify(K, D, (width, height), None)

    mapx2, mapy2 = cv2.fisheye.initUndistortRectifyMap(K,D,(width, height),cv2.CV_32F)
